...
const CTA_BG = ["#C4DE00", "#0B2E4F", "#1E78C8", "#FFFFFF"];
const CTA_FG = ["#0B2E4F", "#FFFFFF"];
const CTA_LIST = [
  ["header", "En-tête"],
  ["hero", "Hero"],
  ["proof", "Témoignages"],
...
function App() {
  ...
  const setCta = (id, key, val) =>
    setTweak("cta", { ...t.cta, [id]: { ...(t.cta && t.cta[id]), [key]: val } });
  ...
  return (
    <div className="app-root" style={{ zoom: t.fontScale }}>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakText label="Vidéo témoignage (URL ou fichier)" value={t.vidVideoUrl} placeholder="uploads/temoignage.mp4" onChange={(v) => setTweak("vidVideoUrl", v)} />

        <TweakSection label="Boutons CTA — couleurs (indépendantes)" />
        {CTA_LIST.map(([id, label]) => (
          <div className="cta-twk" key={id}>
            <div className="cta-twk-lbl">{label}</div>
            <TweakColor label="Fond" value={(t.cta && t.cta[id] && t.cta[id].bg) || "#C4DE00"} options={CTA_BG} onChange={(v) => setCta(id, "bg", v)} />
            <TweakColor label="Texte" value={(t.cta && t.cta[id] && t.cta[id].fg) || "#0B2E4F"} options={CTA_FG} onChange={(v) => setCta(id, "fg", v)} />
          </div>
        ))}
